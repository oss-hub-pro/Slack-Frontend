...
import { FaEdit, FaRegTrashAlt, FaHashtag } from "react-icons/fa"
...
const ChannelItem = (props) => {
    ...
    return (
        <HStack pos="relative" gap={2} onClick={chooseChannel} id="msgTag" bg={props.selected ? "white" : "inherit"} color={props.selected ? color.primary : "interit"} w="full" cursor={"pointer"} px={2} py={1} rounded={"8px"} _hover={{ bg: "#fff9", color: color.primary }}>
            <Text display={'flex'} alignItems={'center'} gap={2} fontSize={'18px'}><FaHashtag/>
            {props.name.slice(0, 20)}
            </Text>
            {
                props.creater == auth._id &&
                <HStack h="full" pos={"absolute"} display={'none'} id="msgTools" right={"4px"} top={0} gap={1} color={'black'}>
